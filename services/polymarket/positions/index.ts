...
export const getUserPositions = async () => {
  ...
  const url = `https://data-api.polymarket.com/positions?sizeThreshold=1&limit=500&sortBy=TOKENS&sortDirection=DESC&user=${userAddress}`;
  try {
    const positions = await http.get(url).json<UserPosition[]>();
    return positions.filter((p) => new Date(p.endDate) > new Date());
  } catch (error) {
